...
const formatMinMaxValue = (v: { value: MinMax }): string =>
  formatMinMax(v.value);

const formatFiller = (f: Filler): string =>
  `${humanize(f.type)}: ${formatMinMax(f.value)}`;
...
export const MaterialConditionLabel: FC<{ condition?: MaterialCondition }> = ({
  condition,
}) => {
...
export const MinMaxValueLabel: FC<{ value?: MinMax; fallback?: string }> = ({
  value,
  fallback,
}) => {
...
export const MaterialPropertyValueFromRecord: FC<
  MaterialPropertyValueFromRecordProps
> = (props) => {
