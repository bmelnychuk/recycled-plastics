import { MaterialData, MinMax } from "@/backend";
import { FC } from "react";
import { MaterialConditionLabel, MaterialTypeLabel } from "./labels";
import { ColorLabel, ColorLabelWithPreview, ColorPreview } from "./MaterialColor";

export type MaterialProperty = keyof MaterialData;
export type MaterialPropertyValue = MaterialData[MaterialProperty];

export const MaterialDataTable: FC<{ material: MaterialData }> = ({ material }) => {
        ...
        <div className="overflow-hidden rounded-lg border">
            <table className="min-w-full divide-y divide-border text-sm">
                <tbody className="divide-y divide-border bg-white">
                    {materialProperties.
                        filter((p) => Array.isArray(material[p]) ? Boolean(material[p].length) : Boolean(material[p]))
                        .map((property) => (
                            <tr key={property}>
...
export const MaterialDataValue: FC<MaterialDataValueProps> = (props) => {
    ...
    switch (props.property) {
        ...
        case "packagingTypes":
            return <>{props.data.packagingTypes?.map((v) => humanize(v)).join(", ")}</>;
        ...
        case "processingMethods":
            return <>{props.data.processingMethods?.map((v) => humanize(v)).join(", ")}</>;
        case "intendedMarket":
        ...
        case "residualMoistureContent":
            return <>{formatMinMaxValue(props.data.residualMoistureContent?.value)}</>;
        case "fillers":
            return <div>
...
const formatMinMaxValue = (value?: MinMax | null): string => value ? formatMinMax(value) : "-";
