import { MaterialColor } from "@/backend";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div
          className={`w-4 h-4 rounded-full ${materialTailwindColorMap[color.toLowerCase()] || "bg-gray-300"
            }`}
        />
      </TooltipTrigger>
      <TooltipContent>
...
export const ColorLabelWithPreview: FC<{ color: MaterialColor }> = ({ color }) => {
  return <div className="flex items-center gap-2">
    <ColorPreview color={color} /><ColorLabel color={color} />
  </div>;
};
