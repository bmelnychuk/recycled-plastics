import { getDemandMaterials } from "@/backend/api";
...
export default async function Page() {
  ...
  const thisWeek = demandMaterials.filter((m) => isAfter(m.createdDate, startDate)).length;
  const buyers = new Set(demandMaterials.map((m) => m.companyId));

  return (
    <div className="p-4 flex flex-col gap-4">
      <DemandHero activeListings={activeListings} countries={countries.size} thisWeek={thisWeek} buyers={buyers.size} />
